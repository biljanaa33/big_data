...
def emit_completed_windows(producer, windows, current_window_start):
    completed_keys = [
        key
        for key in windows
    ...
    for group_type, group_value, window_start in completed_keys:
        ...
        topic = (
            TOPIC_PY_BOROUGH_STATS
            if group_type == "borough"
            else TOPIC_PY_LOCATION_STATS
        )
...
def main():
    ...
    try:
        while True:
            ...
            pickup_time = parse_pickup_time(event.get("tpep_pickup_datetime"))
            ...
            emit_completed_windows(producer, windows, window_start)
        ...
        for group_type, group_value, window_start in list(windows):
            ...
            producer.produce(
                topic,
                key=str(group_value),
                value=json.dumps(finalize(group_type, group_value, window_start, acc)),
            )
